        ...
        <React.Fragment>
            <div className="relative shadow-md rounded-md">
                <div className="w-full min-h-80 bg-gray-200 aspect-w-1 aspect-h-1 rounded-t-md overflow-hidden group-hover:opacity-75 lg:h-80 lg:aspect-none">
                                <Link href={`/assets/${NFTAddress}/${product?.tokenId}${ product? `?price=${product.price}`:""}`}>
                                    <a>
                                        <Image
                                            src={product.image? product.image : product.image_original_url}
                                            alt={product.name}
                            ...
                            <div className="flex justify-between p-4">
                                ...
                                <p className="text-sm font-medium text-gray-900">#{product.tokenId ? product.tokenId: product.token_id}</p>
                            </div>
